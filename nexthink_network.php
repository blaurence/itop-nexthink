...
$sQuery = "(select (mac_addresses group_name distinguished_name)(from device (where device (eq name (string \"".$sName."\")))) )";
...
$oP->add('<div style="max-height:400px; overflow: auto;" id="tbl_network">');
$oP->table($aConfig, $aDetails);
$oP->add('</div>');
$oP->add_ready_script('$("#tbl_network table.listResults").tableHover();');
$oP->add_ready_script('$("#tbl_network table.listResults").tablesorter( { widgets: ["myZebra", "truncatedList"]} );');
